...
| **Q8** | 494.323s ±2.205s | 647.171s ±285.331s | **0.90x** | **1.70%** | **66.7% (2/3)** | **7.433%** | **12.486%** | `cache_hit_template` (AQP Active) |
...
| **Q12** | 505.990s ±20.105s | 700.810s ±276.944s | **0.81x** | **1.70%** | **66.7% (2/3)** | **0.441%** | **0.956%** | `cache_hit_template` (AQP Active) |
...
3. **AQP Engage thành công và kiểm chứng chất lượng thực tế:**
   Thực nghiệm sạch ghi nhận sự kích hoạt AQP thành công tại hai câu truy vấn tiêu biểu là **Q8** và **Q12** (đều đạt 1/3 lượt chạy AQP hoạt động thành công, Fallback Rate 66.7%). Cụ thể, **Q12** hoạt động cực kỳ hoàn hảo với tỷ lệ lấy mẫu thực tế chỉ **1.70%**, mang lại sai số trung bình thực tế rất thấp là **0.441%** (nằm sâu dưới mức trần 5.0% cho phép) và sai số lớn nhất chỉ **0.956%**. Đối với **Q8**, việc kích hoạt AQP mang lại tỷ lệ lấy mẫu thực tế **1.70%**, nhưng ghi nhận sai số thực tế đạt **7.433%** (vượt nhẹ mức trần 5.0% cấu hình). Đây là phát hiện khoa học trung thực và đắt giá: khi ép mở chốt chặn AQP tại quy mô lớn (SF100), phương sai biến động lấy mẫu có thể vượt nhẹ giới hạn cam kết toán học do dung lượng khối lớn. Các truy vấn còn lại đều thực hiện fallback 100% cực kỳ an toàn để bảo vệ tính chính xác kết quả của người dùng lên trên hết.
